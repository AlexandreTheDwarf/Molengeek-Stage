...
class TrainingSession(models.Model):
    ...
    trainer_id = fields.Many2one("res.partner", string="Formateur", required=True)
    ...
    def write(self, vals):
        for session in self:
            if session.state == 'confirmed' and not self.env.user.has_group('base.group_system'):
                raise UserError("Vous ne pouvez pas modifier une session confirmée.")
